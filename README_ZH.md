...
- **搜索与整理** — 在后台索引并显示进度，可跨会话检索元数据、提示词、助手回复、Skill、工具输入和错误；支持项目与 Agent 筛选，可按名称、会话数或最近活动排序项目，并保留带备注的收藏
- **Git 与变更请求证据** — 查看按仓库归属的本地改动、留存 Patch、候选提交与回放编辑锚点；识别任意 Agent 和主机中的 PR/MR/评审链接，区分“会话提到该链接”和“会话通过 CLI 创建”，并可选地关联固定版本的 GitHub PR 或 GitLab MR 快照，明确展示证据质量和可撤销的只读主机授权
