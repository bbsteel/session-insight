...
- **Search and organization** — search metadata, prompts, assistant replies, skills, tool inputs, and errors across sessions while background indexing reports progress; narrow results by project, provider/model, or agent, sort projects by name, session count, or recent activity, and keep bookmarks with notes
- **Coding-plan quota insights** — inspect local-first quota windows, remaining allowances, reset times, and exact, stale, unavailable, or unsupported states for configured coding-plan providers
- **Saved snippets** — save selected terminal text or assistant replies into a searchable library linked back to the source session
- **Git and Change Request evidence** — inspect repository-scoped local changes, retained patches, candidate commits, and replay edit anchors; recognize PR/MR/review-shaped links from any agent and host, distinguish link mentions from CLI creation evidence, and optionally connect fixed GitHub PR or GitLab MR snapshots through revocable read-only host approval
...
| `SI_INDEXER_ENABLED` | `1` | Accepts `1`, `true`, `yes`, `on` (enabled) or `0`, `false`, `no`, `off` (disabled); unset or invalid values fall back to enabled. In a linked worktree, disabled mode leaves the local index stale and disables source watchers, initial indexing, and periodic reconciliation; the primary checkout always forces `1` |
