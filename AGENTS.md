...
- **The primary checkout stays on `main` (required).** Do not switch the primary checkout onto a feature/fix branch for development. Its local `main` moves only by fast-forward from `origin/main` (see Branch and PR Workflow). All development — even when no other agent is active — happens in a dedicated linked worktree, so the primary checkout and its port-8080 instance always track `main`. Exception: the user explicitly asks to work in the primary checkout.
- **Worktree location (required):** create linked worktrees only under the repository's `.worktrees/` directory, named after the branch with `/` flattened to `-` (e.g. `.worktrees/fix-foo` for branch `fix/foo`). Never create sibling checkouts (`../session-insight-*`) or ad-hoc `/tmp` checkouts unless the user explicitly instructs it. Harness-native worktree tools that manage their own placement (e.g. EnterWorktree, which uses `.claude/worktrees/`) are also acceptable. `.worktrees/` must stay in `.gitignore`; verify with `git check-ignore .worktrees` before first use and add it in the same change if missing.
- **Detect existing isolation before creating anything.** If the current checkout's `.git` is a file, you are already in a linked worktree — reuse it; do not nest another one. Only the checkout with a `.git` directory is the primary.
...
- If multiple agents are active, each task works in its own dedicated linked worktree; never use the primary checkout for development writes. Validation of work in progress runs from that worktree (its own random port and `.runtime/` records). The primary checkout validates only what is on `main`.
- The primary checkout owns port 8080 and the default `~/.session-insight` database. In a linked worktree, `./run.sh all` uses an OS-assigned random loopback port on the first run and reuses the same port on subsequent restarts (persisted to `.runtime/session-insight.port`), with its database, PID, log, and discovered URL stored under that worktree's `.runtime/` directory. A non-empty `PORT` env var overrides the persisted port, except `PORT=8080` which is ignored in a linked worktree (reserved for the primary checkout; agent shells often export it). Never treat an HTTP 200 on port 8080 as proof that this worktree is ready.
