...
- Before starting an instance, confirm the port is free (`ss -tlnp | grep <port>`); after starting, check the log for `bind: address already in use` — start.sh prints the URL before binding, so a successful-looking startup message does not mean the server is actually up.
