/** Shared helpers for the production timeline harness runner scripts. */
...
export const FRONTEND_DIR = dirname(dirname(HARNESS_DIR))
export const SCREENSHOT_DIR = process.env.COLLAB_SCREENSHOT_DIR ?? '/tmp/session-insight-ui/collab-timeline'
...
export function buildHarness() {
  ...
  mkdirSync(join(HARNESS_DIR, 'dist'), { recursive: true })
  execFileSync(
    esbuild,
    [
      join(HARNESS_DIR, 'src', 'harness.tsx'),
      '--bundle',
      '--format=esm',
      '--target=es2020',
      '--jsx=automatic',
      `--outfile=${join(HARNESS_DIR, 'dist', 'harness.js')}`,
    ],
    { stdio: 'inherit' },
  )
}
...
export function serveHarness() {
  const server = createServer((req, res) => {
    ...
    const file = join(HARNESS_DIR, path)
    if (!file.startsWith(HARNESS_DIR + sep) || !existsSync(file)) {
      res.writeHead(404)
      res.end('not found')
      return
    }
