...
// - two kind filter capsules, both selected by default, individually toggleable
// - shared 全选 / 全不选 selection capsules
...
const BASE_URL = process.env.SI_VERIFY_URL ?? (() => {
...
const kindPill = (page, label) => panel(page).locator(`button[aria-label="显示${label}"]`)
...
async function openSession(page) {
  ...
  await page.waitForSelector('button[title="交互消息"]', { timeout: 20000 })
  // Positions load async; wait until the header button shows a count.
  await page.waitForFunction(
    () => /^交互消息 \d+$/.test(document.querySelector('button[title="交互消息"]')?.textContent?.trim() ?? ''),
    { timeout: 30000 },
  )
  // Default nav pref auto-opens the panel pinned; open it manually otherwise.
  if ((await panel(page).count()) === 0) {
    await page.locator('button[title="交互消息"]').click()
  }
...
async function run() {
  ...
  await page.addInitScript(() => localStorage.setItem('si-locale', 'zh-CN'))
  const consoleErrors = []
  ...
  try {
    ...
    const btnText = (await page.locator('button[title="交互消息"]').textContent()).trim()
    ...
    // 2. Two filter capsules, both selected by default (fresh profile).
    const userPill = kindPill(page, '用户消息')
    const asstPill = kindPill(page, '助手回复')
    check('用户消息 capsule exists and is selected by default', await userPill.getAttribute('aria-pressed') === 'true')
    check('助手回复 capsule exists and is selected by default', await asstPill.getAttribute('aria-pressed') === 'true')
    ...
    // 6. Individually toggleable: disable 助手回复 → only user rows remain.
    await asstPill.click()
    await page.waitForFunction(
      (n) => document.querySelectorAll('aside div[title^="跳转到终端第"]').length === n,
      user, { timeout: 5000 },
    )
    let chips = await chipCounts(page)
    check('disabling 助手回复 hides assistant rows', chips.assistant === 0 && chips.user === user, JSON.stringify(chips))

    // 7. Disable both → guidance hint.
    await userPill.click()
    check('disabling both shows guidance hint', await panel(page).locator(':text("请选择要显示的消息类型")').isVisible())
    ...
    // 9. Filter state persists across reloads (assistant still off).
    await page.reload()
    await openSession(page) // session selection does not survive a reload
    check('filter choice persists across reload', await kindPill(page, '助手回复').getAttribute('aria-pressed') === 'false' && await kindPill(page, '用户消息').getAttribute('aria-pressed') === 'true')
    chips = await chipCounts(page)
    ...
    await kindPill(page, '助手回复').click() // restore both
    await page.waitForFunction(
      (n) => document.querySelectorAll('aside div[title^="跳转到终端第"]').length === n,
