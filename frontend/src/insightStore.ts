...
export function start(id: string, confirm: boolean, providerId = 0, locale: 'zh-CN' | 'en' = 'zh-CN') {
  const e = entry(id)
  ...
  generateInsight(id, stage => onStage(id, stage), ac.signal, providerId, confirm, locale)
    .then(out => {
